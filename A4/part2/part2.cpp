...
LRESULT CALLBACK part2WndProc(HWND, UINT, WPARAM, LPARAM);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
	...
	wndclass.lpfnWndProc = part2WndProc;//wnd proc to handle windows msgs/commands
	...
	while (GetMessage(&msg, NULL, 0, 0)) {//get message from queue
		...
		if (WaitForSingleObject(closePart2, 0) == WAIT_OBJECT_0)
		{
			PostQuitMessage(0);
		}
	}
	...
	//		if (WaitForSingleObject(closeOtherWindows, 0) == WAIT_OBJECT_0)
	//		{
...
LRESULT CALLBACK part2WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	HDC		hdc;
	PAINTSTRUCT ps;
	RECT	rect;
	HANDLE showWindowPart2;
	...
	case WM_PAINT: //what to do when a paint msg occurs
		...
		DrawText(hdc, TEXT("Part 2"), -1, &rect,
	...
	case WM_COMMAND:
		...
		case (BN_CLICKED):
			...
			showWindowPart2 = CreateEvent(NULL, FALSE, FALSE, TEXT("showWindowPart2"));
			SetEvent(showWindowPart2);

			//MessageBox(hwnd, TEXT("Event Created"), TEXT(""), MB_OK);
		}
		return 0;
